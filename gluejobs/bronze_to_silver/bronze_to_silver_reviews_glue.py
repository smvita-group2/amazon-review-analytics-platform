...
class ReviewsValidator:
    ...
    def validate_helpful_vote(self):
        ...
        if invalid_count > 0:
            ...
            raise ValueError(
                f"Found {invalid_count} negative helpful votes."
            )
...
def main():
    ...
    logger.info(
        "Starting Bronze → Silver Reviews Glue Job"
    )
    ...
    try:
        ...
        logger.info(
            "Bronze → Silver Reviews Glue Job Completed Successfully."
        )
        ...
        logger.exception(
            "Bronze → Silver Reviews Glue Job Failed."
        )
